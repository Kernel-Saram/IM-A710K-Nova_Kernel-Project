...
//#define	NV_VERS_R3200	0xf31ab568 // 20110628 hskim_wifi for mac setting
...
{
  // 20110217 khlee_wifi kernel rpc work for reading wifi mac address from nvram
  //jwjeon100701@DS2  //20100709 thkim_wifi CONFIG_PANTECH_WIFI_MAC_ADDR
	if (vers == NV_VERS_6_1) {
	...
	else if (vers == NV_VERS_9_2) { // 20110301 khlee_wifi newly added for runtime-error
		nv_rpc_ids.prog			= 0x3000000e;
...
int msm_nv_rpc_connect(void)
{
	...
	if (IS_ERR(nv_ep)) {
		...
		/* Initialize rpc ids version 0x00060001 */			
...
static int msm_nv_check_validity(uint32_t item)
{
	...
	switch(item) {
		case	NV_SPARE_1_I:
		case	NV_SPARE_2_I:
       case	NV_WLAN_MAC_ADDRESS_I: 	// 20110621 hskim_wifi for wifi mac address backup
				rc = 0;
				break;
	}
