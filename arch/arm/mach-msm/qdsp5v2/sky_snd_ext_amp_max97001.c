...
static int max97001_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("  [SKY Audio] max97001_probe %d \n", rc);
	return rc;
...
static int max97001_suspend(struct device *dev)
{
	...
	printk("[SKY Audio] AMP suspend(%d)\n", SuspendOk);
...
static void max97001_shutdown(struct i2c_client *client)
{
	...
	snd_extamp_i2c_write(POWER_MANAGEMENT_CTL_REG_ADDR, tExtampInfo.power_man_ctl_reg_val);
	printk("  [SKY Audio] AMP Shutdown for power-off\n");
}
...
{
	int result = 0;
	printk(KERN_ERR "  [SKY Audio] init max97001\n");
	...
	if(result){
		printk(KERN_ERR "  [SKY Audio] init max97001 Fail\n");
	}
}
...
void snd_extamp_api_DeInit(void)
{
	...
	printk(KERN_ERR "  [SKY Audio] DeInit max97001\n");
}
...
int snd_extamp_write_all_reg(extamp_info_t tCurrentExtampInfo)
{
	...
	for(i=0;i<9;i++)	{
		if(status[i])	{
			printk(KERN_ERR "[SKY Audio] snd_extamp_api_SetDevice Fail (%d Reg.)\n", i);
			return 0;
		}
	}

	if(write_status!=0)  {
		printk(KERN_ERR "[SKY Audio] snd_extamp_write_all_reg (Write Status:%x)\n", write_status);
		return 1;
	}
	else {
		printk(KERN_ERR "[SKY Audio] snd_extamp_write_all_reg (No change)  \n");
		return 0;
	}
}
	...
	switch(cad_device)
	{
		...
		if (on) 
		{
	          ...
	          tCurrentExtampInfo.hp_mixers_ctl_reg_val = HPLMIX_INA2 | HPRMIX_INA1;
	           tCurrentExtampInfo.hpl_ctl_reg_val           = ZCD_DIS | SLEW_DIS | HPLM_UNMUTE | HPLVOL_M_1_DB;
	           tCurrentExtampInfo.hpr_ctl_reg_val           = HPGAIN_P_0_DB | HPRM_UNMUTE | HPRVOL_M_1_DB;
		...
		if (on)
		{
	          ...
	          tCurrentExtampInfo.hp_mixers_ctl_reg_val = HPLMIX_INA2 | HPRMIX_INA1;
	          tCurrentExtampInfo.sp_mixer_ctl_reg_val  = SPKMIX_INB2;
	           tCurrentExtampInfo.hpl_ctl_reg_val = ZCD_DIS | SLEW_DIS | HPLM_UNMUTE | HPLVOL_M_14_DB;
	           tCurrentExtampInfo.hpr_ctl_reg_val = HPGAIN_P_0_DB | HPRM_UNMUTE | HPRVOL_M_14_DB;
	          tCurrentExtampInfo.sp_ctl_reg_val        = FFM_FF_MODE  | SPKM_UNMUTE | SPKVOL_M_4_DB;
                   tCurrentExtampInfo.limiter_ctl_reg_val      = THDCLP_DIS |THDT1_1_DOT_4_SEC;
		...
		if (on) 
		{
	          tCurrentExtampInfo.inp_gain_ctl_reg_val  = INADIFF_STEREO_SINGLE  | INBDIFF_DIFFERENTIAL | PGAINA_P_0_DB | PGAINB_P_0_DB;
	          tCurrentExtampInfo.sp_mixer_ctl_reg_val  = SPKMIX_INB2;
	          tCurrentExtampInfo.sp_ctl_reg_val           = FFM_FF_MODE  | SPKM_UNMUTE | SPKVOL_P_3_DB;
                  tCurrentExtampInfo.limiter_ctl_reg_val      = THDCLP_DIS |THDT1_1_DOT_4_SEC;
	          tCurrentExtampInfo.power_man_ctl_reg_val = SHDN_EN  | LPMODE_DIS | SPKEN_EN | HPLEN_DIS | HPREN_DIS | BYPEN_OPEN;
		}
		else
		...
		if (on)
		{
	          ...
	          tCurrentExtampInfo.hp_mixers_ctl_reg_val = HPLMIX_INB2 | HPRMIX_INB2;
	           tCurrentExtampInfo.hpl_ctl_reg_val           = ZCD_DIS | SLEW_DIS | HPLM_UNMUTE | HPLVOL_P_0_DB;
	           tCurrentExtampInfo.hpr_ctl_reg_val           = HPGAIN_P_0_DB | HPRM_UNMUTE | HPRVOL_P_0_DB;
	          tCurrentExtampInfo.power_man_ctl_reg_val = SHDN_EN  | LPMODE_DIS | SPKEN_DIS | HPLEN_EN | HPREN_EN | BYPEN_OPEN;
		}
		else
		{
		...
		break;
	    case SND_SKY_DEVICE_VOICE_HEADSET_RX:
		if (on)
		{
	          tCurrentExtampInfo.inp_gain_ctl_reg_val  = INADIFF_STEREO_SINGLE | INBDIFF_DIFFERENTIAL | PGAINA_P_0_DB | PGAINB_P_0_DB;
	          tCurrentExtampInfo.hp_mixers_ctl_reg_val = HPLMIX_INA2 | HPRMIX_INA1;
...
void snd_extamp_api_SetVolume(uint32_t hp_vol, uint32_t sp_vol)
{
	...
	printk(KERN_ERR "  [SKY Audio] set Volume hp_vol(%d), sp_vol(%d)\n", hp_vol, sp_vol);
...
void snd_extamp_api_SetPreAmpGain(uint32_t gain)
{
	...
	printk(KERN_ERR "  [SKY Audio] set PreAmpGain gain(%d)\n", gain);
...
#endif
	printk("  [SKY Audio] SetVolume. SPK(%d) HPH(%d)\n", tExtampInfo.hpr_ctl_reg_val, tExtampInfo.hpl_ctl_reg_val);
...
/*static*/ void snd_extamp_setpreampgain(extamp_inmode_e inpath, u8 val)
{
	...
	status = snd_extamp_i2c_write(INPUT_GAIN_CTL_REG_ADDR, tExtampInfo.inp_gain_ctl_reg_val);
	printk("  [SKY Audio] snd_extamp_setpreampgain(%d)\n", tExtampInfo.inp_gain_ctl_reg_val);
	if(status)
	{
		printk("  [SKY Audio] snd_extamp_setpreampgain fail(%d)\n", status);
	}
}
