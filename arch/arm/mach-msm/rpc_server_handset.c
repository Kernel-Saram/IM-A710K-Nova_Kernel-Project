...
#include <linux/wakelock.h>		//PZ1567 20110708 earjack kevent wakelock
...
#define FEATURE_WAKELOCK	//PZ1567 20110708 earjack kevent wakelock

#define KEY(hs_key, input_key) ((hs_key << 24) | input_key)

//#define TEST_PI35_ADC_READ
...
static struct wake_lock headset_wakeup;
#endif
...
static int nAdcValue = 0;
static ssize_t show_adc(struct device *dev, struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", nAdcValue);
}

static ssize_t store_adc(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	return 0;
}
#endif
...
#ifdef TEST_PI35_ADC_READ
static DEVICE_ATTR(adc, S_IRUGO | S_IWUSR, show_adc, store_adc);
#endif
...
static int process_subs_srvc_callback(struct hs_event_cb_recv *recv)
{
	...
	printk("key.code = 0x%02X  key.param = 0x%02X\n", be32_to_cpu(recv->key.code), be32_to_cpu(recv->key.parm));
...
{
#ifdef FEATURE_WAKELOCK	
		wake_lock_init(&headset_wakeup, WAKE_LOCK_SUSPEND, "headset_wakeups");
#endif
