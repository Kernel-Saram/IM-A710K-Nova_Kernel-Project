...
#ifndef F_SKYDISP_LAST_FB_CONVERT_xRGB8888_TO_RGB565
#define F_SKYDISP_LAST_FB_CONVERT_xRGB8888_TO_RGB565
#endif
#endif /*FEATURE_SW_RESET_RELEASE_MODE*/
#endif
...
{
#if defined(CONFIG_EF18_BOARD) && defined(CONFIG_PANTECH_ERROR_LOG) // 20100816, kkcho, for EF18 
//#if EF18K_BDVER_GE(EF18K_WS10) || EF18B_BDVER_GE(EF18B_PT10) // #if (BOARD_VER >= EF18K_WS10) // LCDC-interface
...
extern void force_lcd_screen_on(void);
       ...
       force_lcd_screen_on();
#else
extern void force_lcd_screen_on(void);  // MDDI-interface
       force_lcd_screen_on();
#endif
#endif
}
#endif
#if defined(CONFIG_MACH_MSM8X55_EF30S)
#ifdef F_SKYDISP_LAST_FB_CONVERT_xRGB8888_TO_RGB565
extern unsigned int msm_fb_get_offset(void);
extern void msm_fb_set_offset(unsigned int offset);
extern void msm_fb_stop_display(void);
#endif
#endif

#if 1  // ps1 pz1714 20110726 DPM drv timeout log(device name) add
char *kernel_bug_dev_name=NULL;
EXPORT_SYMBOL(kernel_bug_dev_name);

void pantech_errlog_add_dev_name(void)
{
	int length = 0;//test strlen("\n test device name \n");
	// test strlcat(kernel_bug_dev_name, "\n test device name \n", length);

	length = strlen(kernel_bug_dev_name);
	memcpy(&pErrorLog[LogIdx], kernel_bug_dev_name, length);
	LogIdx += length;
	if(kernel_bug_dev_name[length - 1] != '\n') {
		pErrorLog[LogIdx++] = '\n';
	}
}
//EXPORT_SYMBOL(pantech_errlog_add_dev_name);
#endif
...
{
	pantech_errlog_add_dev_name(); // ps1 pz1714 20110726 DPM drv timeout log(device name) add
...
static unsigned int BGCOLOR = 0x0000FF;
static unsigned int FGCOLOR = 0xFFFFFF;
#else
...
#ifdef CONFIG_SKY_FB_24BPP_RGB888 
static void drawglyph(unsigned char *base, int pixels, unsigned int paint,
                      unsigned stride, unsigned *glyph, int bpp)
{
    ...
    for (y = 0; y < 6; y++) {
        for (x = 0; x < 5; x++) {
            if (data & 1) {
                base[(pixels<<2) - 1] = 0;
                base[(pixels<<2) - 2] = (paint & 0xFF0000) >> 16;
                base[(pixels<<2) - 3] = (paint & 0x00FF00) >> 8;
                base[(pixels<<2) - 4] = (paint & 0x0000FF);
            }
            data >>= 1;
            pixels++;
        }
...
#if defined(CONFIG_MACH_MSM8X55_EF30S)
#ifdef F_SKYDISP_LAST_FB_CONVERT_xRGB8888_TO_RGB565
/* [DS3] Kang Seong-Goo, 32bpp -> 16bpp becoz Froyo Upgrade */
#define pack565(r,g,b)  (((r>>3) << 11) | ((g>>2)<<5) | (b>>3))
#define rgb888to565(p)  pack565((p&0x0000FF),((p&0x0000FF00)>>8),((p&0x00FF0000)>>16))
static void convert_to_16bpp(unsigned int offset)
{
    struct fb_info *info;
    ...
    msm_fb_stop_display();
    if (offset == 0) {
        src = (unsigned int *)(info->screen_base);
        dst_tmp = (unsigned int*)(src + offset);
        memcpy(dst_tmp, src, new_offset);
        msm_fb_set_offset(new_offset);
    }

    src = (unsigned int *)(info->screen_base + new_offset);
    dst = (unsigned short *)(info->screen_base);

    for (i = size ; i ; i-=16) {
        dst[i-1] = (short)rgb888to565(src[i-1]); //RGB888 to RGB565
        dst[i-2] = (short)rgb888to565(src[i-2]); //RGB888 to RGB565
        dst[i-3] = (short)rgb888to565(src[i-3]); //RGB888 to RGB565
        dst[i-4] = (short)rgb888to565(src[i-4]); //RGB888 to RGB565
        dst[i-5] = (short)rgb888to565(src[i-5]); //RGB888 to RGB565
        dst[i-6] = (short)rgb888to565(src[i-6]); //RGB888 to RGB565
        dst[i-7] = (short)rgb888to565(src[i-7]); //RGB888 to RGB565
        dst[i-8] = (short)rgb888to565(src[i-8]); //RGB888 to RGB565
        dst[i-9] = (short)rgb888to565(src[i-9]); //RGB888 to RGB565
        dst[i-10] = (short)rgb888to565(src[i-10]); //RGB888 to RGB565
        dst[i-11] = (short)rgb888to565(src[i-11]); //RGB888 to RGB565
        dst[i-12] = (short)rgb888to565(src[i-12]); //RGB888 to RGB565
        dst[i-13] = (short)rgb888to565(src[i-13]); //RGB888 to RGB565
        dst[i-14] = (short)rgb888to565(src[i-14]); //RGB888 to RGB565
        dst[i-15] = (short)rgb888to565(src[i-15]); //RGB888 to RGB565
        dst[i-16] = (short)rgb888to565(src[i-16]); //RGB888 to RGB565
    }
}
#endif
#endif
static void display_log(const char *str, int size)
{
    ...
    t_cx = t_cy = 0;
       ...
       for (k = 0; k < fb_num; k++ ) {
            ...
            j = t_cy * fb_width * 12;
            j <<= 2; // for support 32bpp (RGBX)
    
            while (count--) {                
                base[j++] = (BGCOLOR & 0xFF0000)>>16;
                base[j++] = (BGCOLOR & 0xFF00) >> 8;
                base[j++] = (BGCOLOR & 0xFF);
                base[j++] = 0;
            }
...
#if !defined(FEATURE_SW_RESET_RELEASE_MODE)
             // 20101028, kkcho, fixme for prohibied update during display log-message
              preempt_disable();  
              bust_spinlocks(1);   
#ifdef SKY_MDP_VG_CLEAR_HOLD_CHANGE  
              force_mdp4_overlay_unset(); // 20101028, kkcho, for err-log display in the overlay(surface-view)
#endif
		if(LogIdx == 0 && (pErrorLog[0] != 0x00)) {
			pErrorLog[LOG_SIZE - 1] = 0x00;
			LogIdx = strlen(pErrorLog);
...
#elif defined(CONFIG_MACH_MSM8X55_EF30S)
void pantech_errlog_display_with_errlog(bool bmArm, bool do_panic)
{
#if !defined(FEATURE_SW_RESET_RELEASE_MODE)
#ifdef SKY_MDP_VG_CLEAR_HOLD_CHANGE  
...
#else
#ifdef F_SKYDISP_LAST_FB_CONVERT_xRGB8888_TO_RGB565
		convert_to_16bpp(msm_fb_get_offset()); 
#endif
#endif
...
#if 1 // ps1 pz1714 20110726 DPM drv timeout log(device name) add
	kernel_bug_dev_name = kmalloc(128, GFP_KERNEL);
	memset(kernel_bug_dev_name, 0x00, 128);
#endif
