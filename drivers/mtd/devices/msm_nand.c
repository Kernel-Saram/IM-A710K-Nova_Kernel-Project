...
#ifdef	CONFIG_MTD_QCOM_AMSS
static unsigned CFG0_M, CFG0_A;
static unsigned ecc_buf_cfg_M, ecc_buf_cfg_A;
#endif	// CONFIG_MTD_QCOM_AMSS
...
#ifdef	CONFIG_MTD_QCOM_AMSS
static void set_nand_configuration(char type, struct msm_nand_chip *chip)
{
	if (type == TYPE_MODEM_PARTITION) {
		chip->CFG0 = CFG0_M;
		chip->ecc_buf_cfg = ecc_buf_cfg_M;
	}
	else {
		chip->CFG0 = CFG0_A;
		chip->ecc_buf_cfg = ecc_buf_cfg_A;
	}
}
#endif	// CONFIG_MTD_QCOM_AMSS
...
#ifdef	CONFIG_MTD_QCOM_AMSS
	int	part_type = TYPE_APPS_PARTITION;	

	if (ops->mode == MTD_OOB_AMSS) {
		part_type = TYPE_MODEM_PARTITION;
...
#ifdef	CONFIG_MTD_QCOM_AMSS
	set_nand_configuration(part_type, chip);
#endif	// CONFIG_MTD_QCOM_AMSS
...
#ifdef	CONFIG_MTD_QCOM_AMSS
				{
					if (part_type == TYPE_MODEM_PARTITION) {
						sectordatasize = 512;
					}
					else {
		...
		for (n = start_sector; n < cwperpage; n++) {
			...
			if (ops->datbuf) {
...
#ifdef	CONFIG_MTD_QCOM_AMSS
				{
					if (part_type == TYPE_MODEM_PARTITION) {
						sectordatasize = 512;
                                                //printk(KERN_ERR "[PBC] sectordatasize: %d \n", sectordatasize);
...
#ifdef	CONFIG_MTD_QCOM_AMSS
				if (part_type == TYPE_MODEM_PARTITION) {
					cmd->src = NC01(MSM_NAND_FLASH_BUFFER) + 512;
					sectoroobsize = 15;
				}
				else
#endif	// CONFIG_MTD_QCOM_AMSS
...
#ifdef	CONFIG_MTD_QCOM_AMSS
			if (ops->oobbuf && (part_type != TYPE_MODEM_PARTITION)) 
#else
			if (ops->oobbuf) 
#endif	// CONFIG_MTD_QCOM_AMSS
			{
...
#ifdef	CONFIG_MTD_QCOM_AMSS
			{
                                //printk(KERN_ERR "[PBC] (2) part_type: %d \n", part_type);
				if (part_type == TYPE_MODEM_PARTITION) {
					sectordatawritesize = 512;
...
#ifdef	CONFIG_MTD_QCOM_AMSS
static int
msm_nand_erase_modem(struct mtd_info *mtd, struct erase_info *instr)
{
	int err;
	struct msm_nand_chip *chip = mtd->priv;
	struct {
	...
	set_nand_configuration(TYPE_MODEM_PARTITION, chip);
...
int msm_nand_scan(struct mtd_info *mtd, int maxchips)
{
...
#ifdef	CONFIG_MTD_QCOM_AMSS
	CFG0_A = chip->CFG0;
	ecc_buf_cfg_A = chip->ecc_buf_cfg;
	...
	pr_info("MODEM CFG0 Init  : 0x%08x \n", CFG0_M);
	pr_info("MODEM ECCBUFCFG  : 0x%08x \n", ecc_buf_cfg_M);
...
#ifdef	CONFIG_MTD_QCOM_AMSS
	mtd->msm_erase = msm_nand_erase_modem;
#endif	// CONFIG_MTD_QCOM_AMSS
