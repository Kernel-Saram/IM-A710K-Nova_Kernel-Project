...
#if defined(T_EF18K) || defined(T_EF30S) //p13156@lks - GB Patch

extern int sky_lcdc_display_loadingbar(int ratio); 
#endif //SKY_LCD_LOADBAR_ANDROID
#endif
...
#ifdef CONFIG_FRAMEBUFFER_CONSOLE //20100722, kkcho, 부팅시 우측 상단에 커서 표시 나오는 현상 제거 
...
#if defined(FEATURE_SW_RESET_RELEASE_MODE)
              //if (!sky_sys_rst_is_silent_boot_mode())	// novapex_hsj_20110722__BootOnScreen
#endif
                  //sky_lcd_screen_off(); // for black screen before H/W init
#endif
...
static int msm_fb_register(struct msm_fb_data_type *mfd)
{
	...
	int fbram_offset;
	//p13156@lks 20110531
	// novapex_hsj_20110816__CLK_and_MUSIC
	//struct msm_fb_panel_data *pdata = NULL;
	/*
...
#if defined(T_EF18K) || defined(T_EF30S) //p13156@lks - GB Patch
	var->height = 86.4,	/* height of picture in mm */
	var->width = 51.84,	/* width of picture in mm */
#else
...
#ifdef CONFIG_FB_MSM_LOGO				//p13156@lks 20110531
	...
	if(!sky_sys_rst_is_silent_boot_mode()) {
		...
		sky_lcdc_display_loadingbar(20);

#if defined(FEATURE_SW_RESET_RELEASE_MODE)
	}
#endif

#else //SKY_LCD_LOADBAR_ANDROID
	if (!load_565rle_image(INIT_IMAGE_FILE)) ;	/* Flip buffer */
#endif //SKY_LCD_LOADBAR_ANDROID
#endif
	ret = 0;
...
static int msm_fb_release(struct fb_info *info, int user)
{
 ...
 #ifndef CONFIG_FRAMEBUFFER_CONSOLE
//20100611,  kkcho, 'CONFIG_FRAMEBUFFER_CONSOLE' disable시, 부팅중 LCD_OFF되는 현상 방지. 
//해당 option을 사용하지 않을때만 처리하면 됨..   
	if(mfd->ref_cnt < 0 )
        mfd->ref_cnt = 0;
    if( f_1st_skip)
    {
        //mfd->op_enable = 0;
        printk(KERN_ERR "[SKY_LCD]msm_fb_release: SKIP for Boot_Imgae!\n");
        f_1st_skip = FALSE;
        return TRUE;        
    }
#endif
