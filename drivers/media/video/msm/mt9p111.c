...
#define CONFIG_PANTECH_CAMERA_MT9P111_NO_IMAGE_AFTER_INIT
...
#if 1//EF30S_BDVER_GE(EF30S_WS10)
#else
static struct mt9p111_vreg_t mt9p111_vreg[] = {	
	{
		.name = "gp7",	/* VDD_CAM_1.8V */
		.mvolt = 1800,
	},
	{
		.name = "gp10",	/* CAMERA_CAMA_2.8 */
		.mvolt = 2800,
	},
	{
		.name = "gp4",	/* CAMERA_CAMAF_2,8V */
		.mvolt = 2800,
	},
};
#endif
...
#define CAM_GPIO_MAX   5 /* switch, flash driver IC */
...
#define CAM_VGA_RST_N	2
#define CAM_VGA_STB_N	3
#define CAM_LDO_EN	4 /* switch, external VDD source */
...
static int32_t mt9p111_power_on(void)
{
	...
	if (mt9p111_vreg_out(VDD_CAM_1800, 1))		rc = -EIO;
	msleep(1);
#if defined(CONFIG_MACH_MSM8X55_EF30S)	
	...
	if (mt9p111_gpio_out(CAM_VGA_RST_N, 0))		rc = -EIO;
	msleep(5);
	/* turn on VDD_CAMA_2800 */
#if defined(CONFIG_MACH_MSM8X55_EF30S)	
	if (mt9p111_gpio_out(CAM_LDO_EN, 1))		rc = -EIO;
#elif defined(CONFIG_MACH_MSM8X55_EF18K)
	if (mt9p111_vreg_out(VDD_CAMA_2800, 1))		rc = -EIO;
#endif	
	msleep(5);
	if (mt9p111_vreg_out(VDD_CAMAF_2800, 1))	rc = -EIO;
	if (mt9p111_gpio_out(CAM_VGA_STB_N, 1))		rc = -EIO;
	msleep(15);
	if (mt9p111_gpio_out(CAM_VGA_RST_N, 1))		rc = -EIO;
	msleep(2);
	if (mt9p111_gpio_out(CAM_VGA_STB_N, 0))		rc = -EIO;
	msleep(15);
	if (mt9p111_gpio_out(CAM_5MP_RST_N, 1))		rc = -EIO;
	msleep(10);
#ifdef CONFIG_PANTECH_CAMERA_MT9P111_NO_IMAGE_AFTER_INIT
...
#elif defined(CONFIG_MACH_MSM8X55_EF18K)
	if (mt9p111_vreg_out(VDD_CAMA_2800, 0))		rc = -EIO;
#endif	
	msleep(4);
	if (mt9p111_vreg_out(VDD_CAM_1800, 0))		rc = -EIO;
	msleep(4);
#if defined(CONFIG_MACH_MSM8X55_EF30S)		
...
static int32_t mt9p111_set_led_mode(int32_t led_mode)
{
...
#ifdef CONFIG_PANTECH_CAMERA_FLASH_DRV_IC_AAT3176
	switch (led_mode)
	{
	case 0: // LED OFF
		gpio_request(LED_DATA2, "LED_DATA2");
		rc = gpio_direction_output(LED_DATA2, 0);
		gpio_free(LED_DATA2);
		gpio_request(LED_DATA1, "LED_DATA1");
		rc = gpio_direction_output(LED_DATA1, 0);
		gpio_free(LED_DATA1);			
		SKYCDBG("LED OFF");				
		udelay(500);
		break;
	...
	case 4: //LED 10 MODE
		gpio_request(LED_DATA2, "LED_DATA2");
		rc = gpio_direction_output(LED_DATA2, 0); // movie mode
		gpio_free(LED_DATA2);
		gpio_request(LED_DATA1, "LED_DATA1");
		rc = gpio_direction_output(LED_DATA1, 0);
		udelay(1); // 0.3us ~ 75us
		...
		rc = gpio_direction_output(LED_DATA1, 1);
		udelay(50); // typical 50us, max 75us
		rc = gpio_direction_output(LED_DATA1, 0);
		udelay(1); // 0.3us ~ 75us
	...
	switch(led_mode)
	{
		...
		case 0:
			...
			/* mandatory, settling time */
			msleep(100);

			(void)mt9p111_gpio_out(CAM_FLASH_EN, 0);
			break;
		...
		case 1:
			...
			break;
		case 2:
			(void)mt9p111_gpio_out(CAM_FLASH_EN, 1);
			...
			gpio_request(LED_DATA2, "LED_DATA2");
			rc = gpio_direction_output(LED_DATA2, 1);
			gpio_free(LED_DATA2);		
			SKYCDBG("LED_MODE 01");		 
			break;
		case 3:
...
#define AF_POLL_RETRY	30
	...
	{			
		...
		if(nbrightness <= 0x38) {
			mt9p111_set_led_mode(4); // LED_MODE 01 ON
			led_trigger = TRUE;
		}
...
//	SKYCDBG(" mt9p111_ctrl [ kzalloc ] mt9p111_ctrl=%x\n",mt9p111_ctrl);
	...
	led_auto = 0;
//	mt9p111_sensor_mode = SENSOR_PREVIEW_MODE;
	multi_count = 0;
