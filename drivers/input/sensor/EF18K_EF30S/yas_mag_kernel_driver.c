...
#endif // (YAS_MAG_DRIVER == YAS_MAG_DRIVER_YAS529)
#else
...
//        dbg("%s : schedule_delayed_work(&data->work, time_delay_ms);", __func__);
    }
    else {
        schedule_delayed_work(&data->work, 0);
//        dbg("%s : schedule_delayed_work(&data->work, 0);", __func__);
...
#if defined(PANTECH_EARLY_SUSPEND_SENSOR_OFF) && defined(CONFIG_HAS_EARLYSUSPEND)
void geomagnetic_sensor_early_suspend(struct early_suspend *h)
{
	struct geomagnetic_data *data = i2c_get_clientdata(this_client);
...
void geomagnetic_sensor_late_resume(struct early_suspend *h)
{
	struct geomagnetic_data *data = i2c_get_clientdata(this_client);
...
geomagnetic_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	es.suspend = geomagnetic_sensor_early_suspend;
	es.resume = geomagnetic_sensor_late_resume;
...
#if defined(PANTECH_EARLY_SUSPEND_SENSOR_OFF) && defined(CONFIG_HAS_EARLYSUSPEND)
	if(es.suspend == geomagnetic_sensor_early_suspend)
