...
#include "yas.h"
#if defined(PANTECH_FEATURE_SENSOR) && (YAS_ACC_DRIVER == YAS_ACC_DRIVER_BMA150)
...
/* debug option */
/* -------------------------------------------------------------------- */
//#define SENSOR_ACCELEROMETER_DBG_ENABLE
#ifdef SENSOR_ACCELEROMETER_DBG_ENABLE
#define dbg(fmt, args...)   printk("_SHDBG_ [ACC_DRV] " fmt, ##args)
#else
#define dbg(fmt, args...)
#endif
...
void accelerometer_sensor_early_suspend(struct early_suspend *h);
void accelerometer_sensor_late_resume(struct early_suspend *h);
#endif
...
static int yas_acc_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	es.suspend = accelerometer_sensor_early_suspend;
	es.resume = accelerometer_sensor_late_resume;
...
#if defined(PANTECH_EARLY_SUSPEND_SENSOR_OFF) && defined(CONFIG_HAS_EARLYSUSPEND)
	if(es.suspend == accelerometer_sensor_early_suspend)
...
void accelerometer_sensor_early_suspend(struct early_suspend *h)
{
    struct yas_acc_private_data *data = yas_acc_private_data; 
    struct yas_acc_driver *driver = data->driver;
    ...
    if (data->suspend == 0) {
		if(yas_acc_get_enable(driver) != 0) {
			data->suspend_enable = 1;
#ifndef PANTECH_AVOID_DEADLOCK
			cancel_delayed_work_sync(&data->work);
#endif
			yas_acc_set_enable(driver, 0);
