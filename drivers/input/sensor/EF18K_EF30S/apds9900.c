...
#if defined(CONFIG_EF18K_SENSORS_APDS9900)
#define APDS9900_THRESHOLD_HIGH					23
#define APDS9900_THRESHOLD_LOW					13
#elif defined(CONFIG_EF30S_SENSORS_APDS9900)
#define APDS9900_THRESHOLD_HIGH					30
#define APDS9900_THRESHOLD_LOW					10
#endif
...
#error "Unknown BOARD TYPE for APDS9900"
...
/* Function Proto type */
/* -------------------------------------------------------------------- */
static int apds9900_get_available(void);
...
int apds9900_enable(int type, bool enable)
{
	...
	printk("sensor_type = %d / enable = %d / available = %d\n", type, enable, atomic_read(&apds9900data.available));
	if(atomic_read(&apds9900data.available) == 0) {
		printk("%s : apds9900data is unavailable\n", __func__);
		return -1;
	}

	crntactive = atomic_read(&apds9900data.active);

	if(enable) newactive = crntactive | type;
	else newactive = crntactive & (APDS9900_TYPE_ALL & (~type));

	dbg("current_active = %d / new_active = %d\n", crntactive, newactive);

	if(newactive != crntactive) {
		#ifdef APDS9900_ALWAYSON_ENABLE
		...
		atomic_set(&apds9900data.active, newactive);
	}
...
static int apds9900_suspend(struct device *dev)
{
	...
	if( atomic_read(&apds9900data.irqon) ) {
		set_irq_wake(APDS9900_PS_IRQ, 1);
	}
	atomic_set(&apds9900data.available, 0);
...
static int apds9900_resume(struct device *dev)
{
	...
	atomic_set(&apds9900data.available, apds9900_get_available());
...
int apds9900_ps_measure(axes_t *val)
{
	...
	if(atomic_read(&apds9900data.active) & APDS9900_TYPE_PROXIMITY) {

		// check apds9900 is available.
		if(!atomic_read(&apds9900data.available)) return -1;
		...
		#else
		psval.z = atomic_read(&apds9900data.psval);
		#endif

		if(psval.z >= APDS9900_THRESHOLD_HIGH) psval.x = 0;
		else psval.x = 5;
		psval.y = psval.z;
	}
	else {
	...
	dbg("apds9900_ps_measure : (%d,%d,%d)\n", psval.x, psval.y, psval.z);
	
	memcpy(val, &psval, sizeof(axes_t));
...
int apds9900_ls_measure(axes_t *val)
{
	...
	if(atomic_read(&apds9900data.active) & APDS9900_TYPE_LIGHT) {
	...
	dbg("apds9900_ls_measure : (%d,%d,%d)\n", lsval.x, lsval.y, lsval.z);

	memcpy(val, &lsval, sizeof(axes_t));
...
{
	if(type==APDS9900_TYPE_PROXIMITY) return apds9900_ps_measure(val);
	else if(type==APDS9900_TYPE_LIGHT) return apds9900_ls_measure(val);
	return 0;
}
...
int apds9900_set_irq(bool enable)
{
	...
	atomic_set(&apds9900data.irqon, enable);
...
static void apds9900_work_f(struct work_struct *work)
{
	...
	if(atomic_read(&apds9900data.irqon)) apds9900_set_irqdirection();
...
static int apds9900_set_irqdirection(void)
{
	...
	if(!atomic_read(&apds9900data.available)) {
		ret = -1;
	}
	else {
		// 1. get ps val
	...
	if(ret) {
		...
		atomic_set(&apds9900data.psval, 0);
	}
	else {
		printk("adc = %d\n", adc);
		atomic_set(&apds9900data.psval, adc);
	}
...
static int apds9900_set_thrconfig(u16 thrh, u16 thrl)
{
	int ret = 0;
	u8 enable = 0x2D;
	int active = atomic_read(&apds9900data.active);
...
static int __devinit apds9900_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	// 2 initialize atomic variable
	atomic_set(&apds9900data.psval, 0);
	atomic_set(&apds9900data.lsval, 0);
	atomic_set(&apds9900data.active, 0);
	atomic_set(&apds9900data.irqon, 0);

	// 3. check available
	available = apds9900_get_available();
	atomic_set(&apds9900data.available, available );
	if(!available) goto err_exit;
