 ...
 *
 */
extern char *kernel_bug_dev_name;  // ps1 pz1714 20110726 DPM drv timeout log(device name) add
static void dpm_drv_timeout(unsigned long data)
{
	struct device *dev = (struct device *) data;
	...
	if(kernel_bug_dev_name != NULL)  // ps1 pz1714 20110726 DPM drv timeout log(device name) add
		sprintf(kernel_bug_dev_name, "\n DPM drv timeout: %s (%s) \n", dev_name(dev),(dev->driver ? dev->driver->name : "no driver"));
