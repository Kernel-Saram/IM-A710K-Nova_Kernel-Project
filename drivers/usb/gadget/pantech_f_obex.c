  ...
  struct usb_configuration *c)
{
  printk(KERN_INFO "obex_function_add\n");
	...
	if(_obex_dev){
		printk(KERN_INFO "obex_function_add  1\n");	
		return usb_add_function(c, &_obex_dev->function);
	}else{
		printk(KERN_INFO "obex_function_add 2\n");
		return 0;
	}
}
...
static int pantech_obex_bind_config(struct usb_configuration *c)
{
	...
	printk(KERN_ERR "^^^^ pantech_obex_bind_config 2\n");
	(void)pantech_obex_function_add((struct usb_composite_dev *)_obex_dev, c);	
	printk(KERN_ERR "^^^^ pantech_obex_bind_config 3\n");
	return 0;
}
