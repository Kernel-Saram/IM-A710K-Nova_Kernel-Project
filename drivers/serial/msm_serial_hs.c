...
extern int clk_get_count(struct clk *clk);
#define USE_RX_DMA_WORKAROUND
#endif
...
#ifdef CONFIG_UART2DM_CONSOLE
static void msm_hs_console_putchars(struct uart_port *uport, const char *s, unsigned int count, bool is_printk);
#endif
...
static void msm_hs_submit_tx_locked(struct uart_port *uport)
{
	...
	*tx->command_ptr_ptr = CMD_PTR_LP | DMOV_CMD_ADDR(tx->mapped_cmd_ptr);
    ...
    tx->tx_count = tx_count;
    msm_hs_console_putchars(uport, &tx_buf->buf[tx_buf->tail], tx_count, false);
  }
#endif
}
...
#if 1//def FEATURE_AARM_RELEASE_MODE //GPL_KERNEL SYLEE
void msm_hs_console_set_terminal_onoff(uint32_t onoff)
{
	struct uart_port *uport;
	struct msm_hs_port *msm_uport = &q_uart_port[msm_hs_driver.cons->index];

	uport = &msm_uport->uport;

	if(msm_uport->flag_use_dma == 0){
		if(onoff == 1){
			b_terminal_onoff = 1;
...
#endif
EXPORT_SYMBOL(msm_hs_console_set_terminal_onoff);
#endif
...
#ifdef CONFIG_UART2DM_CONSOLE
	if(msm_uport->flag_use_dma == 0 && b_terminal_onoff == 0) {
		struct circ_buf *tx_buf = &uport->state->xmit;
		uart_circ_clear(tx_buf);
		return;
	}
#endif
...
#ifdef CONFIG_UART2DM_CONSOLE
  msm_hs_write(uport, UARTDM_IRDA_ADDR, 0);
#endif
...
#ifdef CONFIG_UART2DM_CONSOLE
	//fix : it is called before initialing flag_use_dma.
#endif
...
static int __init msm_hs_probe(struct platform_device *pdev)
{
  ...
  if(msm_hs_driver.cons && (msm_hs_driver.cons->index == uport->line)){
		// uart2dm console do not use dma_wake_lock
		wake_lock_destroy(&msm_uport->dma_wake_lock);
    msm_uport->flag_use_dma = 0;
  }else{
    msm_uport->flag_use_dma = 1;
  }
	return ret;
#endif
}
...
#if defined(CONFIG_UART2DM_CONSOLE) && defined(CONFIG_PM)
#define is_console(port)  ((port)->cons && \
        (port)->cons->index == (port)->line)


extern int msm_dmov_disable_channel(unsigned int id);
extern int msm_dmov_enable_channel(unsigned int id);
  
static int msm_hs_suspend(struct platform_device *pdev, pm_message_t state)
{
  struct uart_port *uport;
  struct msm_hs_port *msm_uport;
  unsigned long flags;
  ...
  if(uport){
    if(is_console(uport)) {
      ...
      if(clk_get_count(msm_uport->clk) >= 2)
        clk_disable(msm_uport->clk);
      ...
      msm_dmov_disable_channel(msm_uport->dma_rx_channel);
      if (msm_uport->rx.flush != FLUSH_SHUTDOWN){
        msm_uport->rx.flush = FLUSH_SHUTDOWN;
      }
      spin_unlock_irqrestore(&uport->lock, flags);
...
static int msm_hs_resume(struct platform_device *pdev)
{
  ...
  if(uport) {
    if(is_console(uport)){
      msm_dmov_enable_channel(msm_uport->dma_rx_channel);
      msm_dmov_flush(msm_uport->dma_rx_channel);
...
#if 0
static void msm_hs_power(struct uart_port *uport, unsigned int state,
               unsigned int oldstate)
{
  ...
  switch (state) {
    ...
    case 3:
      ...
      break;
    default:
      printk(KERN_ERR "msm_hs_serial: Unknown PM state %d\n", state);
  }
}
#endif
...
static inline void wait_for_xmitr(struct uart_port *uport, int bits)
{
  ...
  mr = msm_hs_read(uport, UARTDM_MR1_ADDR);

  if((mr & UARTDM_MR1_CTS_CTL_BMSK)  == UARTDM_MR1_CTS_CTL_BMSK)
  {
    unsigned int tmout;
    for(tmout = 1000000; tmout; tmout--) {
      unsigned int isr = msm_hs_read(uport, UARTDM_ISR_ADDR);

      if((isr &  UARTDM_ISR_CURRENT_CTS_BMSK) != UARTDM_ISR_CURRENT_CTS_BMSK)
        break;
      udelay(1);
      touch_nmi_watchdog(); // What is this function doing?
    }
  }
}
  ...
  for(i = 0; i < xfer_cnt; i++)
  {
    ...
    for(j = 0; j < 4; j++)
    {
      ...
      if(*s == '\n')
        ...
        assemble |=  ((*s++) & 0x000000ff) << (j * 8);
        left--;
      }
    }

    if(assemble != 0x00000000)
    {
      wait_for_xmitr(uport, UARTDM_SR_TXRDY_BMSK);
      msm_hs_write(uport, UARTDM_NCF_TX_ADDR, j);
      msm_hs_write(uport, UARTDM_CR_ADDR, CLEAR_TX_READY);
      msm_hs_write(uport, UARTDM_TF_ADDR, assemble);

      if(!is_printk){
        tx_buf->tail = (tx_buf->tail + j) & ~UART_XMIT_SIZE;
        uport->icount.tx += j;
      }
      msm_uport->imr_reg |= UARTDM_ISR_TX_READY_BMSK;
      msm_hs_write(uport, UARTDM_IMR_ADDR, msm_uport->imr_reg);
    ...
    if(left != 0 && *s == '\n')
    {
      assemble = '\r' & 0x000000ff;
      assemble |= ('\n' & 0x000000ff) << 8;

      wait_for_xmitr(uport, UARTDM_SR_TXRDY_BMSK);
      msm_hs_write(uport, UARTDM_NCF_TX_ADDR, 2);
      msm_hs_write(uport, UARTDM_CR_ADDR, CLEAR_TX_READY);
      msm_hs_write(uport, UARTDM_TF_ADDR, assemble);
      if(!is_printk){
        tx_buf->tail = (tx_buf->tail + 1) & ~UART_XMIT_SIZE;
        uport->icount.tx += 1;
      }
      s++; left--;
      msm_uport->imr_reg |= UARTDM_ISR_TX_READY_BMSK;
            ...
            unsigned int count)
{
  ...
  if(co->index < 0 ||  co->index >= UARTDM_NR)
  {
    panic("%s: BUG!\n", __func__);
  }

  msm_uport = &q_uart_port[co->index];
  uport = &msm_uport->uport;


  clk_enable(msm_uport->clk);

  //I rewrite this code because uartdm uses DMA controller.
  msm_hs_write(uport, UARTDM_CR_ADDR, UARTDM_CR_TX_EN_BMSK);
  msm_hs_console_putchars(uport, s, count, true);
...
static int __init msm_hs_console_setup(struct console *co, char *options)
{
  ...
  data |= EIGHT_BPC;
  data |= STOP_BIT_ONE;
//xsemiyas  data |= UARTDM_MR2_ERROR_MODE_BMSK; //xsemiyas_test : (0)char_mode (1)block_mode
  ...
  msm_hs_write(uport, UARTDM_BADR_ADDR, uport->fifosize/8);
  ...
  printk(KERN_INFO "msm_hs_serial: console setup on port #%d\n", uport->line);
	//initialize terminal onoff value
...
#define MSM_HS_CONSOLE &msm_hs_console

#else
#define MSM_HS_CONSOLE NULL
#endif
