...
#if 1//def FEATURE_AARM_RELEASE_MODE //GPL_KERNEL SYLEE
void msm_console_set_terminal_onoff(uint32_t onoff)
{
	if(!console_uart_port) return;

	printk(KERN_ERR "TERMINAL ONOFF[%d]", onoff);

	if(onoff == 1){
		b_terminal_onoff = 1;
		msm_write(console_uart_port, 0x05 , UART_CR);
	}else{
		b_terminal_onoff = 0;
		msm_write(console_uart_port, 0x02 , UART_CR);
	}
}
#endif
