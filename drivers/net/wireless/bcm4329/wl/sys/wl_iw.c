...
)
{
    ...
    error = dev_wlc_intvar_set(dev, "roam_off", dhd_roam);

    if ( error )
    {
        p += snprintf(p, MAX_WX_STRING, "FAIL");
        WL_ERROR(("%s: set dhd_roam error\n", __FUNCTION__));
    }
    else
    {
        p += snprintf(p, MAX_WX_STRING, "OK");
        WL_TRACE(("%s: set dhd_roam = %d\n", __FUNCTION__, dhd_roam));
    }
...
//--> FEATURE_SKY_DS_WIFI_FIXED_DRIVER_RSSI_VALUE
//Original Source Block
//	p += snprintf(p, MAX_WX_STRING, "%s rssi %d ", ssidbuf, rssi);

        p += snprintf(p, MAX_WX_STRING, "rssi %d ", rssi);
//<-- FEATURE_SKY_DS_WIFI_FIXED_DRIVER_RSSI_VALUE
