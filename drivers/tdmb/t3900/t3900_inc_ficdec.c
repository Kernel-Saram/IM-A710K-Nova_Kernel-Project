...
void INC_EXTENSION_001(ST_FIB_INFO* pFibInfo, ST_FIC_DB* pstFicDB)
{
  ...
  for(ucIndex = 0; ucIndex < pstHeader->ITEM.bitLength-1;)
  {
    ...
    {
      if(((pstFicDB->aucSubChID[ucLoop] & FIC_BIT_MASK) == (ucSubChId & FIC_BIT_MASK)) 
    ...
    for(ucLoop = 0; ucLoop < pService->ITEM.bitNumComponent; ucLoop++, ucFrame+=2)
    {
      ...
      switch(ucTMID) 
      {
        ...
        case TMID_2 :
          ...
          nChanPos = INC_CHECK_SERVICE_CNT8(pstFicDB->aucSubChID, (INC_UINT8)pMscnFidc->ITEM.bitSubChld, pstFicDB->ucSubChCnt, FIC_BIT_MASK);
        ...
        default :
          ...
          for(ucIndex = 0; ucIndex < (INC_INT32)pstFicDB->ucServiceComponentCnt; ucIndex++){
            ...
            {
              if((pstFicDB->aucSubChID[ucSubCnt] & FIC_BIT_MASK) != pstFicDB->aucSubChPackMode[ucIndex])  continue;
...
void INC_EXTENSION_003(ST_FIB_INFO* pFibInfo, ST_FIC_DB* pstFicDB)
{
  ...
  for(ucIndex = 0 ; ucIndex < pstHeader->ITEM.bitLength-1; )
  {
    ...
    {
      if(((pstFicDB->auiServicePackMode[ucLoop] & FIC_BIT_MASK) == (pTypeInfo->ITEM.bitScid & FIC_BIT_MASK)) && 
        ((pstFicDB->aucSubChPackMode[ucLoop] & FIC_BIT_MASK) == (pIdInfo->ITEM.bitSubChId & FIC_BIT_MASK))) 
      {
...
void INC_EXTENSION_008(ST_FIB_INFO* pFibInfo, ST_FIC_DB* pstFicDB)
{
  ...
  ST_TYPE_0*      pstType;
  ST_MSC_BIT*     pstMsc;
  ST_MSC_SHORT*   pstMscShort = INC_NULL;
  ST_MSC_LONG*    pstMscLong = INC_NULL;
  ...
  for(ucIndex = 0 ; ucIndex < pstHeader->ITEM.bitLength-1; )
  {
    ...
    if(ucLSFlag) {
      ...
      if(cResult != -1){
        ...
        pstFicDB->aucSubChPackMode[cResult], pstFicDB->ucSubChCnt, FIC_BIT_MASK);
        if(cResult != -1) {
      ...
      if(!pstMscShort->ITEM.bitMscFicFlag){
        cResult = INC_CHECK_SERVICE_CNT8(pstFicDB->aucSubChID, 
        pstMscShort->ITEM.bitSUBnFIDCId, pstFicDB->ucSubChCnt, FIC_BIT_MASK);
        if(cResult >= 0) {
          pstFicDB->aucServiceComponID[cResult] = pstMsc->ITEM.bitScIds;
          pstFicDB->aulServiceID[cResult] = ulSerId;
        }
...
void INC_EXTENSION_010(ST_FIB_INFO* pFibInfo, ST_FIC_DB* pstFicDB)
{
  ST_FIG_HEAD*    pstHeader;
  ST_TYPE_0*      pstType;
  ST_UTC_SHORT_INFO*  pstUTC_Short_Info = INC_NULL;
  ST_UTC_LONG_INFO*   pstUTC_Long_Info = INC_NULL;
  ST_UTC_INFO*        pstUTC_Info = INC_NULL;
...
#ifdef USER_APPLICATION_TYPE
    pFicDb->astUserAppInfo.uiUserAppType[ucLoop] = -1;
...
#ifdef FEATURE_INC_FIC_UPDATE_MSG
    TDMB_MSG_INC_BB("[%s] pChInfo->ulRFFreq [%d]\n", __func__, (unsigned int)pChInfo->ulRFFreq);
    TDMB_MSG_INC_BB("[%s] pChInfo->uiEnsembleID [0x%x]\n", __func__, pChInfo->uiEnsembleID);
    TDMB_MSG_INC_BB("[%s] pChInfo->ucSubChID [0x%x]\n", __func__, pChInfo->ucSubChID);
    TDMB_MSG_INC_BB("[%s] pChInfo->ucServiceType [0x%x]\n", __func__, pChInfo->ucServiceType);
    TDMB_MSG_INC_BB("[%s] pChInfo->uiStarAddr [0x%x]\n", __func__, pChInfo->uiStarAddr);
    TDMB_MSG_INC_BB("[%s] pChInfo->uiBitRate [%d]\n", __func__, pChInfo->uiBitRate);
    TDMB_MSG_INC_BB("[%s] pChInfo->uiTmID [%d]\n", __func__, pChInfo->uiTmID);

    TDMB_MSG_INC_BB("[%s] pChInfo->ucSlFlag [%d]\n", __func__, pChInfo->ucSlFlag);
    TDMB_MSG_INC_BB("[%s] pChInfo->ucTableIndex [%d]\n", __func__, pChInfo->ucTableIndex);
    TDMB_MSG_INC_BB("[%s] pChInfo->ucOption [%d]\n", __func__, pChInfo->ucOption);
    TDMB_MSG_INC_BB("[%s] pChInfo->ucProtectionLevel [%d]\n", __func__, pChInfo->ucProtectionLevel);
    TDMB_MSG_INC_BB("[%s] pChInfo->uiDifferentRate [0x%x]\n", __func__, pChInfo->uiDifferentRate);
    TDMB_MSG_INC_BB("[%s] pChInfo->uiSchSize [0x%x]\n", __func__, pChInfo->uiSchSize);
#endif
    break;
  }
