...
//EI33 0506 netber test시 fic 받지 않기 위해 추가
...
#define INC_FIFO_DEPTH    (1024*20)  //chunk size가 64 FIFO overflow 발생하여 10->20 변경
#else //EBI2
...
typedef struct _tagST_OUTPUT_EWS
{
  ...
  INC_UINT8 ucMsgGovernment; //聘 芟
  INC_UINT8 ucMsgID;         //聘 호
  ST_DATE_T stDate;          //絿

  INC_INT8 acKinds[4];       //楹?
  INC_UINT8 cPrecedence;     //耳?
  INC_UINT32 ulTime;         //楹??
  INC_UINT8 ucForm;          //楹?
  INC_UINT8 ucResionCnt;     //楹?
