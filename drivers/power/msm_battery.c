...
#define FEATURE_SKY_VBATT_RPC // battery level 랑 charger state를 따로 읽지 않고 한번에 읽어오게 RPC 추가해 구현

#if defined(CONFIG_MACH_MSM8X55_EF30S) || defined(CONFIG_MACH_MSM8X55_EF18K)
#define FEATURE_SKY_CHANGE_PM_POLICY
#endif
...
#else
#if defined(CONFIG_MACH_MSM8X55_EF30S) //FEATURE_SKY_CHARGER_USB
#define BATTERY_LOW            	3200
#else
#define BATTERY_LOW            	3200
#endif
...
#ifndef FEATURE_SKY_CHANGE_PM_POLICY
...
#if defined(CONFIG_MACH_MSM8X55_EF30S) && defined(FEATURE_SKY_CHG_LOGO)
				  ...
				  union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
        ...
        case POWER_SUPPLY_PROP_CAPACITY:
                ...
                DBG_LIMIT("%s: power supply = %s, batt_capacity = %d\n",
...
#endif    
{
	...
	if (rc < 0) {
		pr_err("%s : ERROR. SKY_VBATT_READ_PROC RPC error = %d, \n",
				__func__, rc);
		return rc ;
	}
...
static void msm_batt_sky_update_psy_status(void)
{
	...
	// 충전완료 후 충전기에서 충전 전류를 감소시켜 level이 떨어지는 문제점
	// 충전완료 후 방전 시 충전기가  연결되어 있는 상태라면 완충으로 display되도록 생기에서 요청
	if( msm_batt_info.batt_status == POWER_SUPPLY_STATUS_FULL )
	{
		static int once_more_flag = 0;
		if( batt_charging == 1 || batt_charging == 2) //충전 (AC, USB)
		{
		...
		{
			if( once_more_flag == 1 ) // 충전 -> 방전시 바로 battery level을 정상 표시하면 바로 뚝 떨어질 수 있으므로 다음 event까지  100% 유지
			{
				printk(KERN_INFO "Display once more batt_capacity 100\n");
				msm_batt_info.batt_capacity = 100;
				once_more_flag++;
			}
			else if( once_more_flag >= 2 ) // 2번째는 99%
			{
				if( msm_batt_info.batt_capacity < 99 ) // 99% 이상인 경우엔 그대로 표시
				{
					printk(KERN_INFO "Display once more batt_capacity 99\n");
					msm_batt_info.batt_capacity = 99;
				}
				once_more_flag = 0; // 3번째 부터 정상표시
			}
		}
	}
	...
	{
		if( batt_permil > 1150 ) //115%가 넘는 경우 over voltage
		{
		...
		else
		{
			...
			if( msm_batt_info.batt_capacity >= 100 ) // batt_permil>=960
		...
		//실제 msm_batt_info.battery_level값은 app단에서 사용되지 않음
	...
	if (msm_batt_info.charger_valid != batt_charging)
	{
		...
		switch (msm_batt_info.charger_valid)
		{
			case 0:
				{
					...
					{
						msm_batt_info.current_chg_source &= ~AC_CHG;
						printk(KERN_INFO "%s() : AC charger removed\n",__func__);
			...
			case 2:
            	...
            	printk(KERN_INFO "msm_psy_batt => batt_level(permil) = %u batt_volt = %u batt_charging = %u\n", batt_permil,batt_volt,batt_charging );
		power_supply_changed(&msm_psy_batt);
       }
...
#ifdef FEATURE_SKY_CHANGE_PM_POLICY
...
#ifdef FEATURE_SKY_BATT
		/* early suspend 상태에서 15% 이하인 경우 call back이 불리도록 함 */
...
//sleep중에는 battery 정보가 처리되지 않으니 resume시 업데이트 하도록 함
// Eclair 버전 이후 sleep시 BATTERY_VOLTAGE_BELOW_THIS_LEVEL로 rpc가 등록되어 15% 이하이고 battery level이 떨어지는 경우에만 event가 발생됨
#ifdef FEATURE_SKY_BATT
...
#else //FEATURE_SKY_CHANGE_PM_POLICY
...
#endif //FEATURE_SKY_CHANGE_PM_POLICY
#endif

#ifndef FEATURE_SKY_BATT // fuel gauge사용하므로 filter기능은 disable
...
#else
// dummy function is added by jwcha100804@DS2
void msm_batt_sky_fake_usb_info(uint32_t onoff) 
{
	printk(KERN_ERR "CONFIG_BATTERY_MSM_FAKE is enabled\n");
	printk(KERN_ERR "You can't use this option. Please reconfig and build kernel\n");
}
EXPORT_SYMBOL(msm_batt_sky_fake_usb_info);
#endif  /* CONFIG_BATTERY_MSM_FAKE */
